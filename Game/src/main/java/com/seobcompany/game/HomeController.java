...
public class HomeController {
	...
	public String home(Locale locale, Model model) {
		DBCommon<Player> dbCommon = new DBCommon<Player>("c:\\tomcat(apache)\\game.db", "player");
...
//		return "redirect:/my_account";
		...
		UpdateCard updatechangecard = new UpdateCard();
		updatechangecard.updateCardQuery(idx);
		
		return "redirect:/my_account";
	}
	
	@RequestMapping(value = "/delete_card", method = RequestMethod.GET)
	public String deleteCard(Locale locale, Model model, HttpServletRequest request) {
//		DBCommon<Player> dbCommon = new DBCommon<Player>("c:\\tomcat(apache)\\game.db", "player");
//		HttpSession session = request.getSession();
//		HashMap<String, String> userData = dbCommon.detailsData(new Player(), (Integer)session.getAttribute("user_idx"));
//		int idx = Integer.parseInt(userData.get("idx"));
...
//		return "redirect:/my_account";
		...
		deletecard.deleteCardQuery(idx);
		session.invalidate();
		return "redirect:/";
	}
	...
	public String doBattle(Locale locale, Model model, HttpServletRequest request, @RequestParam("select1") int select1, @RequestParam("select2") int select2) {
		...
		model.addAttribute("random_number", random.nextInt(9999999));
